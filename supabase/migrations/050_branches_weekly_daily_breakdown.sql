...
--
-- Adds a per-day cup breakdown to branches_weekly_summary so the Weekly tab
-- can show each of the club's last 6 operating days individually, not just
-- the 6-day totals. `daily` is a jsonb array ordered oldest to newest, one
-- entry per operating day in the window: {date, total_cups, coach_cup_total}.
