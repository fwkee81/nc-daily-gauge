...
--
-- Branches page Monthly Leaderboards ("Most New 5-Day Sign-ups", "Most
-- 30-Day Activity", "Highest Avg Coach's Cup") were crediting a coach with
-- events from customers whose nc_club_id didn't match the coach's own
-- nc_club_id — e.g. a coach's "Coach" field can point to someone registered
-- at a different branch. Every board now only counts events (signups,
-- renewals, check-ins) that happened at the club the coach is actually
-- registered under, matching the same fix already applied to Daily
-- Report's Coach's Cup.
