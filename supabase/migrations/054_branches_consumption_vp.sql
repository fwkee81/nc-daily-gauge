...
--
-- Adds "Consumption VP" (quantity out × VP, excluding voided movements) to
-- all three Branches tabs, per club: today vs. previous operating day on
-- Daily, the club's own 6-operating-day window on Weekly, and the calendar
-- month on Monthly — same shape as NC Metrics' Consumption VP stat.
--
-- Adding a column changes the return signature, which CREATE OR REPLACE
-- can't do — drop each function first.
...
drop function if exists branches_weekly_summary(date);
...
drop function if exists branches_monthly_summary(date);
