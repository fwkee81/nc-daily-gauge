-- Add "Foodie" as a new Finance income category, positioned right after
-- "Power Cup". Postgres won't let a brand-new enum value be referenced by
-- other DDL in the same transaction it was added in ("unsafe use of new
...
-- STEP 1
alter type finance_category add value if not exists 'Foodie' after 'Power Cup';
