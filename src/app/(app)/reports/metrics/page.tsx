...
export default async function MetricsPage({
  ...
  searchParams: Promise<{ month?: string; date?: string; club?: string; tab?: string }>;
}) {
  const coach = await getCurrentCoach();
  if (!coach) redirect("/onboarding");

  const { month: monthParam, date: dateParam, club: clubParam, tab } = await searchParams;
  ...
  const [
    ...
    supabase.rpc("monthly_inventory_out", { p_month: `${month}-01`, p_club_id: clubId }),
  ...
  return (
    <MetricsClient
      ...
      tab={tab === "demographics" ? "demographics" : tab === "weekly" ? "weekly" : "monthly"}
      ...
      inventoryOut={inventoryOutRes.data ?? []}
      weeklyTotals={
        weeklyTotalsRes.data?.[0] ?? {
          operating_days: 0,
          window_start: null,
          window_end: null,
          total_cups: 0,
